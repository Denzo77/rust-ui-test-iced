use iced::{
    alignment, theme,
    widget::{button, checkbox, column, container, row, scrollable, text, text_input, Text},
    Application, Color, Command, Element, Font, Length, Renderer,
};
...
impl Application for Checklist {
    ...
    fn view(&self) -> iced::Element<'_, Self::Message, Renderer<Self::Theme>> {
            ...
            column(
                self.entries
                    ...
                    .map(|(id, entry)| {
                        entry
                            .view(id)
                            .map(move |message| Self::Message::Entry { id, message })
                    })
                    .collect(),
            )
            .spacing(10)
        ...
        let content = column!(title, input, entries).spacing(20).max_width(800);
        ...
        match &self.state {
            EntryState::Idle => {
                let checkbox = checkbox(
                    &self.description,
                    matches!(self.checked, EntryChecked::Checked),
                    |b| {
                        if b {
                            EntryMessage::Checked(EntryChecked::Checked)
                        } else {
                            EntryMessage::Checked(EntryChecked::Unchecked)
                        }
                    },
                )
                .width(Length::Fill);
            ...
            EntryState::Editing => {
                let text_input = text_input(
                    "Entry name",
                    &self.description,
                    EntryMessage::DescriptionEdited,
                )
